...
// initial size if given size is 0
#define VECTOR_INIT_ALLOC 4
...
typedef struct {
  int log_len;                          // number of elems stored in vector
  int alloc_len;                        // allocated space can store this number of elems
  int elem_size;                        // size of single lement in vector
  void* data;                           // pointer on actual data where elements are stored
  VectorFreeFunction free_func;         // function that frees single element in vector
} vector;
