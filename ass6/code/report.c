                ...
                AccountAmount amount)
{
  ...
  sem_wait(&bank->report_transfer_lock); 
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&bank->report_transfer_lock); 
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  sem_wait(&bank->last_worker_check); // untill the last worker has come we can't send this report
  bank->workers_done_today++;
  int err = 0;
  if (bank->workers_done_today == bank->total_workers) { // am I the last one today?
    Report *rpt = bank->report;
    assert(rpt); Y;
    if (rpt->numReports >= MAX_NUM_REPORTS) {
      // We've run out of report storage for the bank
      bank->workers_done_today = 0;
      for(int i = 0; i < bank->total_workers; i++) {
        if (i != workerNum) sem_post(bank->ready_for_tomorrow + i);
      } 
      return -1;
    }
    /*
    ...
    */
    ...
    bank->workers_done_today = 0;
    // here is the last worker getting ready for the next day
    for(int i = 0; i < bank->total_workers; i++) {
      if (i != workerNum) sem_post(bank->ready_for_tomorrow + i);
    }
    sem_post(&bank->last_worker_check);
  } else {
    sem_post(&bank->last_worker_check);
    sem_wait(bank->ready_for_tomorrow + workerNum);
  }   
