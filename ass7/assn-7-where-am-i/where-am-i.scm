...
(define (partition pivot ls cmp)
  (if (null? ls) '(() ())
      (let ((split-of-rest (partition pivot (cdr ls) cmp)))
	(if (cmp (car ls) pivot)
...
(define (quicksort ls cmp)
  (if (<= (length ls) 1) ls
      (let ((split (partition (car ls) (cdr ls) cmp)))
...
(define (intersection-points circles)
  (if (> 2 (length circles)) '()
    (append 
      (apply append 
        (map (lambda (circle) (intersect (car circles) circle))
          (cdr circles)))
      (intersection-points (cdr circles)))))

;; 2
(define (distance-product from to_list)
  (apply * 
    (map (lambda (point)
            (dist from point))
      (remove from to_list))))
...
(define (sort-points rate_list)
  (quicksort rate_list pair_cmp))
...
; (define (sum-point points)
;   (if (null? points) '(0 0)
;     (add (car points) (sum-point (cdr points)))))

; (define (average_pairs points)
;   (let ((jami (sum-point points))
;         (sigrdze (length points)))
;     (list
;       (/ (x jami) sigrdze)
;       (/ (y jami) sigrdze))))

(define (average-point points)
  (let ((avg (average_pairs points)))
    (list (distance-product avg points)
      avg)))

; (display (where-am-i '(2.5 11.65 7.75) '((0 0) (4 4) (10 0)) )) (newline)

(define (average_pairs points)
  (list (/ (apply + (map (lambda (c) (car c)) points)) 
          (length points))
        (/ (apply + (map (lambda (c) (cadr c)) points)) 
          (length points))))
;;7
